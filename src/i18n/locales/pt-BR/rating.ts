export const ptBRRating = {
  title: 'Avaliar',
  organizer: 'Avalie o Organizador',
  athletes: 'Avalie os Atletas',
  xp: '+25 XP',
  sections: {
    about: 'SOBRE O ORGANIZADOR',
    comment: 'COMENTÁRIO (OPCIONAL)',
    host: 'HOST'
  },
  ratings: {
    excellent: 'EXCELENTE',
    veryGood: 'MUITO BOM',
    good: 'BOM',
    regular: 'REGULAR',
    poor: 'A MELHORAR'
  },
  anonymous: 'Avaliação anônima',
  anonymousDesc: 'Seu nome não aparece',
  next: 'Próximo: Avalie 4 Atletas',
  continue: 'Continuar para Atletas',
  pending: 'Pendente',
  success: 'Avaliação enviada com sucesso!',
  host: {
    title: 'Avalie o Organizador',
    xp: '+25 XP',
    context: 'Host da partida no Arena Central - Quinta 19h30',
    about: 'SOBRE O ORGANIZADOR',
    commentOptional: 'COMENTÁRIO (OPCIONAL)',
    commentSample: 'Pontual, organização perfeita.',
    anonymousTitle: 'Avaliação anônima',
    anonymousSubtitle: 'Seu nome não aparece'
  },
  players: {
    title: 'Próximo: Avalie 4 Atletas',
    pending: 'Pendente',
    continue: 'Continuar para Atletas'
  },
  status: {
    savedTitle: 'Avaliação salva',
    savedSubtitle: 'Pedro recebeu {{hostStars}} estrelas'
  }
};

