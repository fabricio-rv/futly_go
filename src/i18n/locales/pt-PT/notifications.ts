export const ptPTNotifications = {
  title: 'Notificações',
  tab: 'ATIVIDADE',
  time: {
    now: 'agora',
    minutes: 'min',
    hours: 'h',
    days: 'd',
    yesterday: 'ontem',
    daysAgo: 'há X dias'
  },
  types: {
    success: 'Sucesso',
    error: 'Erro'
  },
  messages: {
    requestAccepted: 'Solicitação aceita!',
    requestRejected: 'Solicitação recusada.',
    ratingSubmitted: 'Avaliação enviada!',
    partitaConfirmed: 'Partida confirmada!',
    playerJoined: 'Um novo jogador entrou!',
    playerLeft: 'Um jogador saiu da partida.'
  },
  errors: {
    failed: 'Não foi possível processar a solicitação.',
    ratingFailed: 'Não foi possível enviar a avaliação.'
  },
  common: {
    success: 'Sucesso',
    error: 'Erro'
  },
  requests: {
    accepted: 'Solicitação aceita!',
    rejected: 'Solicitação recusada.',
    processError: 'Não foi possível processar a solicitação.',
    acceptedTitle: 'Solicitação aprovada',
    rejectedTitle: 'Solicitação recusada',
    acceptedByYouInMatch: 'Aceitaste esta solicitação no teu jogo.',
    rejectedByYouInMatch: 'Recusaste esta solicitação no teu jogo.',
    acceptedForRequester: 'A tua solicitação foi aceite no jogo.',
    rejectedForRequester: 'A tua solicitação foi recusada no jogo.'
  },
  rating: {
    sent: 'Avaliação enviada!',
    sendError: 'Não foi possível enviar a avaliação.',
    modalTitle: 'Avaliar',
    commentPlaceholder: 'Comentário opcional'
  },
  subtitle: 'ATIVIDADE',
  account: {
    title: 'Notificações da conta'
  },
  loading: 'A carregar notificações...',
  empty: 'Nenhuma notificação por enquanto.',
  actions: {
    accept: 'Aceitar',
    reject: 'Recusar',
    rateNow: 'Avaliar agora',
    cancel: 'Cancelar',
    send: 'Enviar'
  },
  recentActions: {
    title: 'Ações recentes',
    empty: 'Sem ações recentes.'
  },
  newParticipationRequest: 'Novo pedido de participação',
  requestedToJoin: 'pediu para participar em',
  evaluationAvailable: 'Avaliação disponível',
  actionTypes: {
    ratePlayer: 'Avaliar jogador',
    rateHost: 'Avaliar anfitrião',
    ratingSent: 'Avaliação enviada',
    matchCreated: 'Partida criada',
    participationConfirmed: 'Participação confirmada',
    ratingPending: 'Avaliação pendente'
  },
  requestStatus: {
    sent: 'Solicitação enviada',
    approved: 'Solicitação aprovada',
    rejected: 'Solicitação recusada',
    cancelled: 'Solicitação cancelada',
    approvedByYou: 'Solicitação aprovada por ti',
    rejectedByYou: 'Solicitação recusada por ti'
  }
};
