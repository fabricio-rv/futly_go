  ...
  return (
    <View className="rounded-[14px] border p-[1px] flex-row" style={{ backgroundColor: matchTheme.colors.lineStrong, borderColor: matchTheme.colors.lineStrong, minHeight: compact ? 32 : 42 }}>
      {options.map((option) => {
        ...
        return (
          <Pressable key={option.id} onPress={() => onChange(option.id)} className="flex-1 items-center justify-center" style={{ backgroundColor: active ? matchTheme.colors.ok : matchTheme.colors.bgSurfaceA, borderRadius: 12, minHeight: compact ? 30 : 40 }}>
