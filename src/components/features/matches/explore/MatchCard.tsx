...
export function MatchCard({ partida, onPress, rightAction, bannerPalette = defaultBanner }: MatchCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <LinearGradient colors={bannerPalette} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} className="h-24 px-[14px] py-3 justify-center" style={{ overflow: 'hidden', height: 70, justifyContent: 'center', paddingHorizontal: 14 }}>
        <View className="absolute right-0 top-1/2 -mt-7" style={{ alignItems: 'flex-end' }}>
          <Text variant="number" className="text-[60px] opacity-10" style={{ color: '#FFFFFF' }} numberOfLines={1}>
            {partida.modality.replace(/\d+/g, '').toUpperCase()}
          </Text>
        </View>
