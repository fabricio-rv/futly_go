...
export function mapMatchRowToPartida(
  ...
  const isHost = options.currentUserId
    ? match.created_by === options.currentUserId
    : false;
  const when = new Date(`${match.match_date}T${match.match_time}`);
  const isFinished =
  ...
  return {
    ...
    modality:
      match.modality === "campo"
        ? "campo11"
        : (match.modality as Partida["modality"]),
    location:
      [match.district, match.city].filter(Boolean).join(" - ") ||
      match.city ||
      "Sem local",
    ...
    occupiedSlots,
    totalSlots,
    status: isFinished ? "done" : isHost ? "host" : "open",
